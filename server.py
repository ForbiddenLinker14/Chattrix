...
async def join(sid, data):
    ...
    await sio.emit(
        "room_admin_update",
        {"room": room, "admin": current_admin, "locked": current_lock_state},
        to=sid,
    )
...
async def set_room_lock(room: str, request: Request):
    ...
    if room not in ROOM_ADMINS or ROOM_ADMINS[room] != user:
        return JSONResponse(
            {"error": "Only room admin can lock/unlock the room"}, status_code=403
        )

    # Update lock state
    ROOM_LOCK_STATES[room] = locked
    ...
    print(f"🔒 Room {room} {'locked' if locked else 'unlocked'} by admin {user}")
...
async def get_room_lock(room: str):
    ...
    ROOM_LOCK_STATES[room] = locked  # Sync with memory
...
async def get_app_version():
    ...
    return {
        "current_version": {
            ...
            "name": "8.5",  # Latest available version name
        },
        "min_required_version": {
            "code": 13,  # Minimum allowed version code - CHANGE THIS TO CONTROL UPDATES
            "name": "8.5",  # Minimum allowed version name - CHANGE THIS TO CONTROL UPDATES
        },
    }
